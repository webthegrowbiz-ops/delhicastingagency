...
export const metadata = {
  title: "Popular & Featured Actors | Delhi Casting Agency (DCA)",
  description:
    "Discover featured and popular talent available for selected casting and entertainment opportunities with strong screen presence and audience engagement.",
};

export default function PopularActorsPage() {
  const category = getCategoryBySlug("popular")!;
  const actors = getActorsByCategory("popular");
  const otherCategories = ACTOR_CATEGORIES.filter((c) => c.slug !== "popular");
  ...
  return (
    <main className="bg-white min-h-screen text-[#111111]">
          ...
          <div className="flex flex-col md:flex-row md:items-end justify-between gap-3 mb-6 pb-4 border-b border-gray-200">
            <div>
              <span className="text-xs uppercase tracking-widest text-[#d4af37] font-semibold flex items-center gap-1.5 mb-1.5">
                ...
                Celebrated &amp; High-Demand Performers
              </span>
              <h2 className="text-2xl sm:text-3xl font-extrabold text-[#111111] tracking-tight">
                Popular Actor Profiles
              </h2>
            </div>
      ...
      {/* Category CTA */}
      <CTASection
        eyebrow="Popular Talent Inquiries"
        title="Looking to Cast Recognizable Faces for Campaigns or Feature Leads?"
        description="Delhi Casting Agency coordinates celebrity bookings, endorsement contracts, and principal lead casting."
        buttonLabel="Contact Talent Managers"
        buttonHref="/contact/"
      />
    </main>
  );
