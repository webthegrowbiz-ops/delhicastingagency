...
interface ChildArtistCardProps {
  childArtist: ChildArtist;
}

export function ChildArtistCard({ childArtist }: ChildArtistCardProps) {
  return (
    <Link
      href={`/child-artists/profile/${childArtist.id}`}
      className="group block cursor-pointer"
    >
      {/* Editorial Portrait Image */}
      <div className="relative aspect-[3/4] w-full overflow-hidden bg-[#F7F7F5]">
        <Image
          src={childArtist.mainImage}
          alt={`${childArtist.name} - ${childArtist.categoryLabel}`}
          fill
          unoptimized
      ...
      <div className="mt-3 text-left">
        ...
        <p className="text-xs text-[#666666] tracking-wide uppercase font-medium mt-0.5 truncate">
          {childArtist.location.split("/")[0].trim()}
        </p>
      </div>
    </Link>
  );
